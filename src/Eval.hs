...
module Eval ( runEval, evalTrip, RGBTup, showRGBTup ) where
...
runEval evalM size seed = do
    ...
    evalStateT stateM $ randomList seed
...
evalExp (EVar XVar) x _ = asks (\(w, _) -> fromIntegral x / fromIntegral w)
evalExp (EVar YVar) _ y = asks (\(_, h) -> fromIntegral y / fromIntegral h)
